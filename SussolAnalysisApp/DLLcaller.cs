    ...
    public class DLLcaller
    {
        ...
        public void GetNestedResults(AlgorithmName algorithm, string dataset, string filetype) {
            ...
            switch (algorithm){
                ...
                case AlgorithmName.CANOPY:
                    for (var param1 = (int)Analyses.MinenMaxWaarden.canopyNmin; param1 < (int)Analyses.MinenMaxWaarden.canopyNmax + 1; param1++)
                    {
                        for (var param4 = (int)Analyses.MinenMaxWaarden.canopyT1min; param4 < (int)Analyses.MinenMaxWaarden.canopyT1max + 1; param4++)
                        {
                            for (var param3 = (int)Analyses.MinenMaxWaarden.canopyT2min; param3 < (int)Analyses.MinenMaxWaarden.canopyT2max + 1; param3++)
                            {
                                for (var param2 = (int)Analyses.MinenMaxWaarden.canopyMaxCandidatesMin; param2 < (int)Analyses.MinenMaxWaarden.canopyMaxCandidatesMax + 1; param2++)
                        {
                           
                              
                                    feature = "Number of clusters: " + param1 + " Number of candidates: " + param2 + " T2 distance: " + param3 + "T1 distance: " + param4;

                                    if (param1 != 0) {
                                        var model = sus.canopyModeller(datasets, param1.ToString(), param4.ToString(), param3.ToString(), param2.ToString()).ToString();
                                        jObject = JObject.Parse(model);
                                        JArray items = (JArray)jObject["clusters"];
                                        int length = items.Count;
                                        // filename = "analyseRapport__parameter_is_" + feature + "_value_is_" + o + ".txt";
